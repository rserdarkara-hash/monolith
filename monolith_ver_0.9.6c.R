# Monolith: Advanced Spatial Analysis Dashboard v0.9.6c
...
apply_interpolation <- function(data, target_var, method, grid_p, aux_vars, lags, method_params, l, prefix) {
  ...
  res <- tryCatch({
    if(method == "OK") {
      ...
      for(av in aux_vars) {
        ...
        if (!is.null(kr_res$warn)) {
          res$log_msg <- paste0(res$log_msg, kr_res$warn)
        }
      }
    ...
    res$log_msg <- paste0("Error in apply_interpolation: ", e$message)
    res
  })
                                              ...
                                              fluidRow(
                                                ...
                                                column(9,
                                                  ...
                                                  div(style = "position: relative;",
                                                      ...
                                                      uiOutput("pca_main_plot_container")
                                                  ),
                                                  hr(),
                                                  conditionalPanel("output.pca_ready == 'yes'",
                                                    h4("PCA Results"),
...
server <- function(input, output, session) {
  ...
  output$pca_main_plot_container <- renderUI({
    p <- pca_plot_obj()
    if (inherits(p, "plotly")) {
      plotly::plotlyOutput("pca_main_plotly_out", height = "500px")
    } else {
      plotOutput("pca_main_static_out", height = "500px")
    }
  })
  ...
  output$pca_main_static_out <- renderPlot({
    p <- pca_plot_obj()
    req(p)
