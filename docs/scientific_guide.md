...
**Cross-Validation Holdout Convention (full-row removal):** CK cross-validation removes the **entire held-out row** — the primary observation *and* its co-located covariate observations — from the co-kriging system for each fold (`gstat.cv(..., remove.all = TRUE)`). The gstat default (`remove.all = FALSE`) removes only the primary variable and is the right convention when secondary data are exhaustively available (e.g. a cheap collocated sensor raster); in this application the covariates are co-sampled laboratory measurements, so at real prediction locations on the grid CK has no covariate values and predicts them jointly through the LMC. Scoring CV with the covariates left in would evaluate the model under an information regime the map never enjoys, and would be inconsistent with RK/RFK, whose leave-one-out procedure removes full rows. CK metrics are therefore honest with respect to the actual prediction task (and correspondingly lower than under the gstat default).
...
The FNN rule above drives the **sidebar suggestion** and the per-locality resolution table. The prediction grid that an interpolation *run* builds is derived independently, per locality, from that locality's own boundary area:

$$R_{actual} = \min\left(1000,\ \max\left(5,\ \sqrt{A_{boundary} / 100{,}000}\right)\right) \quad \text{[m]}$$

that is, a target of roughly 100,000 grid cells inside each locality's boundary, floored at 5 m and capped at 1000 m. The bound keeps the cost of predicting at every grid node finite regardless of extent, and because it is evaluated per locality it preserves the Auto (Per Locality) intent: a compact, densely sampled locality receives a fine grid and a large one a coarser grid. The FNN density figure still governs the **dynamic buffer** width (Section 3.2).

In **Fixed (Manual)** mode the slider value is used verbatim, subject to one guard: if it would generate more than ~4 million candidate cells over the boundary's bounding box it is coarsened to that limit and a note is written to the run log, because `terra::as.points()` materialises every bounding-box cell before the boundary clip.

**The Auto rule is self-contained: it does not read the manual-resolution slider.** Earlier versions floored *every* Auto resolution at 10% of that slider's value. The slider is hidden outside Fixed mode, and in Auto modes it carries the **global** FNN recommendation, so on a dataset whose localities are far apart (large bounding-box diagonal → large global recommendation) the floor silently coarsened the grid of every compact locality — a per-locality quantity overridden by a global-scale one through a control the user could not see. Runs made before this correction may therefore have used a coarser Auto grid, with correspondingly different exported GeoTIFFs and classified-area tables, than the density-derived value.
...
- **Robust Fallback Engine:** To prevent mathematical crashes (e.g., singular matrix inversion) on difficult datasets, the auto-fit engine screens a grid of candidates instead of trusting a single fit. It fits four theoretical models, Sph (Spherical), Exp (Exponential), Gau (Gaussian) and Mat (Matern, &nu; = 1.5), from each of four starting ranges (`max_dist/10`, `/5`, `/4`, `/2`), and keeps the eligible candidate with the lowest weighted-least-squares error, preferring converged fits over singular or non-converged ones. If no candidate qualifies, a heuristic Spherical model is returned rather than an error: nugget-dominated (95% nugget, range `max_dist/10`) when the empirical nugget already exceeds 80% of the sample variance, otherwise structured (80% partial sill, range `max_dist/2`). An empirical variogram with fewer than five lag bins skips fitting altogether and takes the structured fallback directly, because `gstat::fit.variogram` can crash on one that short.
- **Matern smoothness is fixed, not estimated.** The candidate search fits nugget, partial sill and range by weighted least squares, but leaves the Matern smoothness parameter &nu; (kappa) at its starting value of **1.5** — the Matern 3/2 model — for every candidate. `gstat::fit.variogram` only estimates &nu; when explicitly asked to (`fit.kappa = TRUE`), and it is deliberately not asked here: a free smoothness parameter would give the Matern candidate one more degree of freedom than the other three and make the cross-candidate residual-sum-of-squares comparison that selects the winner an unequal contest. Read the "Mat" candidate as "Matern with &nu; = 1.5", not as a fitted-smoothness Matern. If the empirical variogram suggests a markedly smoother or rougher process near the origin, use **Manual Tuning** to compare the families directly: Gaussian is the smoothest at the origin and Exponential the roughest, with Spherical and Matern (&nu; = 1.5) between them, so the short-lag behaviour can be bracketed without freeing a smoothness parameter.
- **Candidates are screened on the PRACTICAL range, not on gstat's range parameter.** Each fitted candidate must fall inside a sanity window (between one hundredth and twice the largest empirical lag distance) before it can win. That test is applied to the **practical range** — the distance at which the model reaches ~95% of its sill — because gstat's range parameter *a* means a different ground distance in every family: the practical range is *a* for Spherical (which reaches the sill exactly at *a*), but 3*a* for Exponential, &radic;3·*a* for Gaussian and ≈4.75·*a* for Matern with &nu; = 1.5. Testing the raw *a* therefore judged families by different standards — an Exponential structure extending three times further than a Spherical one passed the same test — so the pool of acceptable candidates was composed on a family-dependent criterion rather than on fit quality. Converting to the practical range first makes the window mean the same physical distance for every candidate. This affects only which candidates are *eligible*; the winner among them is still the lowest weighted-least-squares error, with converged fits preferred over flagged ones.
...
* **Deduplicated Search Set**: The search also removes co-located points (identical projected coordinates at centimetre precision) exactly as the interpolation run does. Without this, a held-out point's co-located twin predicts it at distance zero — a guaranteed exact hit for every candidate power — so the search would be scored on artificially inflated skill and on a different point set than the run that consumes the result. This is a no-op for datasets without co-located samples.
...
* **Deduplicated Search Set**: Co-located points are removed before the GCV search, matching the interpolation run's point set. `fields::Tps` treats exact replicates through its pure-error/replicate handling, which shifts the GCV curve — so without deduplication the stored lambda would be optimized on a different point set than the run that consumes it. No-op for datasets without co-located samples.
* **Interpretation Caveat (fixed lambda only)**: The same selection-optimism applies here as for IDW (Section 4.2), but *only* when lambda is held fixed. A lambda produced by the "Optimize TPS Lambda" button (or typed into the slider by hand) is stored once and then reused unchanged in every cross-validation fold, so it was chosen on data that includes each fold's held-out point: the reported Model Performance metrics for a fixed-lambda TPS run are mildly optimistic. **Auto (GCV) mode has no such reuse**: each fold refits its own lambda by generalized cross-validation on that fold's training points alone, so the smoothing parameter is never informed by the point it is scored on. If you optimize lambda mainly to inspect the GCV curve, consider returning the slider to Auto for the run whose metrics you intend to report.
...
All fold assignments use a fixed seed (`12345`) for reproducibility. An optional **Repeated CV** checkbox re-runs the cross-validation under additional fold assignments and reports each metric as mean ± SD across realizations in a separate table, without altering the reference run or the map; see Section 9.1 for what the spread does and does not mean.
...
**The two performance tables share one metric dictionary.** The **Model Performance** table (interpolation cross-validation) reports, in this order: RMSE, NRMSE (%), MAE, R² (Corr), R² (NSE/Trad), Bias (ME), Lin's CCC (Agree), RPD (Prec), RPIQ, SMAPE (%), Moran's I and Moran p. The **uploaded-prediction** table (an ML prediction column against the observed values) reports the same statistics under the same labels, with two deliberate differences: it adds NMAE (%) and reports MBE in the *predicted − observed* direction described above, and it carries **no Moran's I / p**. That omission is substantive, not an oversight — Moran's I here is a diagnostic of *cross-validation* residuals, i.e. of errors the spatial model makes at held-out locations; the residuals of an externally supplied prediction column were produced by a model this application did not fit and did not resample, so a spatial-autocorrelation test on them would not be a validation statistic of anything the dashboard controls.
...
- **Moran's I (Spatial Autocorrelation of Residuals):**
  ...
  **What the table reports.** The Model Performance table shows Moran's I and, in a separate column, its two-sided p-value; hovering the I cell reveals that row's null expectation, E[I] = −1/(n − 1). Reading I against zero is wrong: under the no-autocorrelation null the statistic is centred slightly *below* zero (−0.034 at n = 30, −0.010 at n = 100), so a marginally positive I is not by itself evidence of clustered errors. The p-value comes from `spdep::moran.test` under the **normality assumption** (`randomisation = FALSE`) and is **two-sided** (`alternative = "two.sided"`, a deliberate departure from spdep's one-sided default): this quantity is a diagnostic, and strongly *negative* residual autocorrelation (a checkerboard error pattern, typically from over-smoothing) is as much a misspecification signal as positive clustering. When the spdep neighbour search fails and the all-pairs 1/d fallback computes I instead (possible only at n ≤ 500), that weighting carries no sampling distribution, so the p column reports `NA*` while I and E[I] are still shown.

  **`NA*` means "not computable", never "no structure".** Both Moran cells fall back to the `NA*` marker when the statistic could not be computed for the point set at all: fewer than three cross-validated points, no coordinate columns on the cross-validation object, or a failure of the neighbour search that the fallback could not rescue (the fallback is not attempted above 500 points). This is a *missing measurement*, not a finding of spatial randomness; the Run Log records which of those conditions applied.
...
- **Classification Performance (Accuracy, Cohen's Kappa, Weighted Kappa, MCC):** Observed and predicted values are binned into classes and compared as a confusion problem. For **agronomical classes**, the bin intervals are left-closed `[low, high)`, identical to the map classification (`terra::classify(..., right = FALSE)`), so a value lying exactly on a class boundary receives the same class in the performance tables and on the classified map. **Quartile** binning uses the conventional right-closed intervals on the observed quartiles.
  ...
  **Quartile breaks come from the observed values and are unbounded at the ends.** The four classes are defined by the quartiles of the **observed** column, and that single set of breaks is then applied to *both* the observed and the predicted values, because a confusion matrix requires one common class definition for the two vectors (breaks re-derived separately from each would compare different classes and make kappa meaningless). The outermost intervals are extended to −∞ and +∞, so a prediction that falls outside the observed range is assigned to Q1 or Q4 rather than being dropped from the table. This keeps every point in the comparison, at the cost that extreme over- or under-predictions are clamped into the end classes by construction: the confusion matrix cannot show *how far* beyond the observed range a prediction went, only that it landed in the outermost class. Read the continuous metrics (RMSE, Bias) alongside the kappa table when extrapolation is a concern.
...
**Pooled variance-explained scores are inflated when localities differ in level.** The "Total (Combined)" row runs the same `perform_cv` routine on the pooled residuals, so its **R² (NSE/Trad)** and **R² (Corr)** are measured against the *pooled* mean of all localities. Where localities sit at genuinely different levels (different mean salinity, different mean nutrient status), that between-locality variance enters the total sum of squares and the model is credited with explaining it, even if within each locality it explains nothing. The pooled row can therefore report a markedly higher R²/NSE than *every* per-locality row above it. This is an aggregation artifact of the pooling, not extra skill (the same effect makes a regression fitted across heterogeneous groups look stronger than any within-group fit). Error-scale metrics in that row (RMSE, MAE, Bias) are unaffected by the choice of centre and remain directly interpretable. **Judge model skill on the per-locality rows**; read the pooled row as a summary of overall error magnitude. A within-locality-centred variant of the score is deliberately not computed: it would be a third quantity to explain, and the per-locality rows already answer the question honestly.
...
**The RFK trend forest is not hyperparameter-tuned.** The random forest that fits the RFK trend is grown with `randomForest`'s regression defaults apart from the number of trees, which is the user-set **RF Trees (`ntree`)** control: `mtry = max(⌊p/3⌋, 1)` predictors tried per split and `nodesize = 5` observations in a terminal node, with `importance = TRUE` and `keep.inbag = TRUE` set for the variable-importance panel and the infinitesimal-jackknife uncertainty term. No search over `mtry`, `nodesize` or tree depth is performed, and none of those values adapts to the sample size. This is a deliberate asymmetry with the Classification Suite (Section 10.6), whose learners do have a tuning registry: the RFK trend is one component of a two-step estimator whose residual variogram is refitted inside every cross-validation fold, so adding an inner tuning search would multiply an already heavy cost, and untuned forest defaults are the standard reference configuration in the regression-kriging literature (Hengl et al. 2015). The practical consequence is that RFK's trend is a *reasonable default* forest rather than an optimized one; where the covariate set is large relative to n, `mtry = p/3` may be conservative. Adding trend tuning would be a design change, not a parameter tweak.

**RFK residuals are out-of-bag while the trend surface is in-bag.** The residuals whose variogram RFK fits and kriges are formed from the forest's **out-of-bag** predictions (`randomForest$predicted`), whereas the trend surface predicted at grid cells uses the **full in-bag ensemble**. This asymmetry is deliberate and standard (Hengl et al. 2015): in-bag training residuals of a random forest are near zero by construction, so a residual variogram fitted to them would be pure nugget and the residual-kriging step would contribute nothing. Two consequences follow, and they are the price of a usable residual model. First, `trend + kriged residual` is not an exact decomposition of the measured value at a sample point, because the two terms come from different ensembles of trees. Second, out-of-bag residuals are slightly **inflated** relative to the in-bag fit, so the residual variogram's sill — and with it the residual component of the RK/RFK uncertainty — is conservative. RK has no equivalent asymmetry: its `lm` residuals and its trend surface come from the same fitted model.
...
* **Geostatistical Engine (RK, RFK, CK):** Before launching multivariate spatial interpolations (Regression Kriging, Random Forest Kriging, or Co-Kriging), the system performs a Variance Inflation Factor (VIF) check on the selected auxiliary variables. If variables with a VIF > 10 are detected, an interactive modal halts the process, advising the user to "Auto-Drop and Continue" or force execution. If auto-dropped, the variables are strictly purged from both the interpolation algorithms and downstream diagnostic reports (e.g., Variable Importance plots). The modal's choice is honoured end-to-end: choosing to keep collinear covariates disables iterative pruning inside the engines entirely (the collinearity is still reported, just not acted upon), identically in the interpolation and classification paths. Independently of that choice, **numerically constant covariates are always excluded** before the VIF iteration — a constant carries no regression information and would make the correlation matrix singular. "Constant" is judged **relative to each covariate's own magnitude** (standard deviation below ~10⁻⁸ of the column's largest absolute value), never against an absolute variance floor: an absolute threshold silently discards legitimately small-unit covariates — clay expressed as a 0–1 fraction, normalized indices, ratios, or variables in km or Mg can all carry real signal at variances below 10⁻⁶ — and, because constants are dropped even under "Keep All", the user's override could not rescue them. The relative criterion drops only columns whose values differ in noise digits, which is the case the correlation matrix genuinely cannot support.
  ...
  * *Note on the singular-matrix fallback:* the VIF itself is obtained by inverting the covariate correlation matrix. When the covariates are so tightly related that the inversion fails outright (an exact or near-exact linear dependency), no VIF exists, and the engine falls back to the pairwise correlation matrix: it takes the most strongly correlated pair and drops the member with the **higher mean absolute correlation against all remaining covariates** — the more globally redundant of the two — repeating until the matrix inverts. Exact ties are broken on the alphabetically later variable name. Both rules exist so the outcome is a property of the data alone: the earlier version dropped whichever member of the pair appeared first among the columns, so re-ordering the columns of the uploaded file could change which covariate survived and therefore the fitted model. This path is reached only when `solve()` genuinely fails; the ordinary VIF > threshold pruning above is unaffected.
...
* **Context sensitivity (all gates and the Predictor Ranks):** correlation structure is scope-dependent — two covariates can be nearly collinear across the whole dataset yet independent within one locality (or vice versa, a Simpson's-paradox-type reversal). All three screens therefore run on the data the model will actually fit: the interpolation gate and the sidebar **Predictor Ranks (Correlation)** list use the localities selected in the Context panel (the ranks are stamped with their scope and sample count), and the classification gate uses the module's resolved spatial scope (localities and/or polygons). A recorded Drop/Keep decision is reset whenever the method, the covariate set, or the locality selection changes.

### 8.2 Multivariate Outlier Screening (Mahalanobis Distance)

The PCA panel's outlier diagnostic computes the Mahalanobis distance of each observation in the space of the retained principal components. Because PC scores are uncorrelated by construction, their covariance is the diagonal matrix of squared component standard deviations, and the distance over the retained components equals the full-space Mahalanobis distance restricted to that subspace. Components with numerically zero variance (which appear when PCA is force-executed on collinear inputs) are excluded, both from the distance and from the degrees of freedom of the &chi;<sup>2</sup> reference line (97.5% quantile, df = number of retained components).

**The estimator is the classical one, and that is a real limitation.** The centre and scatter are the ordinary sample mean and covariance, which the candidate outliers themselves help to define. A single extreme point inflates the covariance in its own direction; a small cluster of outliers can inflate it enough to pull their distances back under the threshold. This is the classical **masking** effect, and the standard remedy is a high-breakdown estimator such as the Minimum Covariance Determinant (MCD, Rousseeuw & Van Driessen 1999), which the panel deliberately does **not** use — adding it would introduce a further modelling dependency for a screening plot. The panel therefore names its estimator in the title and subtitle. Read a flagged point as "worth inspecting", and treat an *unflagged* point in a suspicious cluster with caution rather than as evidence of normality.

### 8.3 Partial Correlations

A partial correlation between two variables removes the linear influence of *k* control variables from both before measuring what remains. The estimator follows the **`ppcor` conventions** (the same package convention the reported p-values already assume), and it is method-dependent:
...
* **Spearman:** every variable — targets *and* controls — is **rank-transformed first**, and the partial correlation is the product-moment correlation of the *rank* residuals. This matters: a Spearman correlation of raw-value residuals is not a partial rank correlation, because the residualization it inherits is a least-squares fit on untransformed values. Since Spearman's coefficient is by definition the Pearson correlation of ranks, the partial version must residualize the ranks.
* **Kendall:** no residualization analogue exists for tau, so the partial coefficients come from inverting the Kendall tau matrix, &rho;<sub>ij·rest</sub> = −P<sub>ij</sub> / &radic;(P<sub>ii</sub>P<sub>jj</sub>) with **P** the inverse tau matrix. For a single control this reduces to Kendall's classical first-order partial tau.

Significance uses the partial degrees of freedom *n* − 2 − *k* (Pearson/Spearman t-statistic) or a normal approximation with effective sample size *n* − *k* (Kendall). A variable listed both as a target and as a control is silently removed from the control set (controlling for itself would yield null residuals), and if the residualization or matrix inversion fails the table aborts with an explicit message rather than falling back to raw correlations under a "partial" label.

### 8.4 Spatial Cross-Correlogram

The cross-correlation of two variables **as a function of the distance separating sample points**. Both variables are standardized to zero mean and unit variance, and the empirical cross variogram &gamma;<sub>12</sub>(*h*) is computed with the same distance binning as every other variogram in the application (lag width from Section 4.1's lag logic, over projected metric coordinates):
...
Under second-order stationarity the cross variogram and the cross-covariance are related by &gamma;<sub>12</sub>(*h*) = C<sub>12</sub>(0) − [C<sub>12</sub>(*h*) + C<sub>21</sub>(*h*)]/2, so for standardized variables the plotted spatial cross-correlation is

<div style="text-align:center;"><i>&rho;<sub>12</sub>(h) = r<sub>12</sub> − &gamma;<sub>12</sub>(h)</i></div>

with *r*<sub>12</sub> the ordinary non-spatial correlation of the two variables (shown as a dashed reference line). See Goovaerts (1997, Section 4.2.3) and Isaaks & Srivastava (1989, Ch. 4).

**How to read it.** A curve starting near *r*<sub>12</sub> at short lags and decaying toward zero means the two variables co-vary *locally* — they share spatial structure at the scale where the curve is still above zero, which is the co-regionalization Co-Kriging exploits. A flat curve at roughly *r*<sub>12</sub> across all lags means the association carries no distance structure; a flat curve near zero means the variables are spatially unrelated at the sampled scales. The distance at which the curve reaches zero is the practical range of the cross-structure.

**Caveats.**
* Only the **symmetric** part of the cross-covariance is estimable here. Bins are omnidirectional, so the pairs (*i*,*j*) and (*j*,*i*) fall in the same bin: unlike a time series, an unordered point set has no "leads" and "lags", and there is no negative-lag half of the plot. Directional (anisotropic) cross-structure is not resolved; see Section 5.1 for the univariate anisotropy diagnostic.
* Each point is a bin average whose reliability follows its pair count. Bins with fewer than 30 pairs are greyed (Journel & Huijbregts' conventional minimum), and point size encodes the count.
* Sampling noise can push an individual bin marginally outside [−1, 1]; the values are moment estimates, not constrained coefficients.
* With `Spearman` or `Kendall` selected as the method, both variables are rank-transformed before standardization and the curve is a rank-based (Spearman-type) cross-correlogram, labelled as such. Kendall has no distance-binned analogue of its own.
...
*Note: this panel replaced a lagged cross-correlation (CCF) plot whose "lag" counted rows of the uploaded table. For a spatial point table that ordering is arbitrary — re-sorting the upload changed the published curve — and the &plusmn;1.96/&radic;n significance bands it drew assume a stationary sequence. Distance-binned cross-correlation is the spatially meaningful replacement.*
...
To ensure "scientific reproducibility" across identical runs, every cross-validation fold assignment, both the random k-fold partition and the Spatial Block k-means clustering, uses a fixed seed (`CV_FOLD_SEED = 12345`).
* **Where to change:** `make_cv_folds` inside `spatial_metrics.R` is the single source of fold assignments for all engines; `perform_kriging_loocv` additionally seeds its per-fold Random Forest draws (RFK). Both go through the shared two-sided RNG sandbox `with_seed()` (`spatial_vgm.R`), which restores the caller's random state afterwards, so seeding a helper never perturbs anything that runs after it.
* **How to change:** Change the `CV_FOLD_SEED` constant (`spatial_metrics.R`) to score the run on a different partition.

**How to read a single fold realization.** By default every reported cross-validation metric comes from **one** fold assignment, not from an average over repeated partitions. Because the fold split is itself random (for random k-fold and Spatial Block; LOOCV is deterministic and unaffected), at moderate sample sizes the spread of a metric across alternative splits can be of the same order as the difference between two interpolation methods, so a small RMSE gap between methods should not be read as a decisive ranking. The fixed seed makes the comparison **paired** — every method is scored on exactly the same folds, which removes fold luck from the *comparison* even though it remains in each absolute number.

**Repeated cross-validation (optional).** The **Repeated CV** checkbox in the Spatial Engine panel quantifies that remaining spread directly. When it is on, each engine re-runs its own cross-validation under additional fold assignments (seeds `CV_FOLD_SEED + 1`, `+ 2`, …) and the Scientific Analysis tab gains a **Fold-Realization Stability** table reporting every displayed error metric as *mean ± standard deviation* across realizations. Points to note:

* **Nothing else changes.** Realization 1 is the reference run (seed `CV_FOLD_SEED`), so the Model Performance table, the residual maps, the CV-residual write-back and the interpolated surface are identical whether the option is on or off. The feature only *adds* a second table.
* **Only the partition varies.** A repeat re-draws the fold assignment and nothing else: the data, the variogram policy, the neighbourhood, the trend model and every model-fitting seed are held fixed, so the reported SD isolates fold-assignment variance rather than mixing it with model-fitting randomness.
* **Deterministic plans never repeat.** Wherever the resolved plan is leave-one-out (Standard LOOCV at any n; Auto at n ≤ 50; Spatial Block below n = 30) the partition is unique, so that locality contributes a single realization and the run log says so. In a pooled ("Total (Combined)") row such a locality is carried unchanged into every repeat, which is exact for a deterministic plan and keeps the pooled repeat rows built from the same locality set as the pooled row above them.
* **How to read the SD.** It is the resolution of the comparison, not an error bar on a physical quantity: if two methods' RMSE values differ by less than this SD, the ranking between them is fold luck. Reporting the mean ± SD of a repeated k-fold is the conventional way to present resampling-based skill estimates (Kohavi 1995; Molinaro et al. 2005).
* **Cost.** One additional full cross-validation pass per repeat, per locality, per surface. For RK/RFK, which refit the trend and the residual variogram inside every fold, five realizations means roughly five times the cross-validation time; the prediction surfaces are computed once regardless.
* **Moran's I is not repeated.** It is a spatial diagnostic of one residual field and is reported for realization 1 only, in the Model Performance table.
...
The **Export Class Zones** button on the Map Viewer toolbar writes the displayed classified surface as a GIS vector layer (`build_class_zone_sf`, `spatial_pipeline.R`). It is a change of representation, not of method, and is deliberately built from the same two calls the on-screen map and the Area Coverage table already make:

* **Geometry.** `terra::classify(rcl_mat, right = FALSE)` — the identical classification, including the left-closed interval convention that keeps the polygons consistent with the map — followed by `terra::as.polygons(dissolve = TRUE)`. One feature per class present in the surface. Polygon boundaries are therefore **cell boundaries**: the zones inherit the interpolation grid resolution and are not smoothed, contoured, or generalised, because any of those would move the boundary away from where the model actually places it.
* **Area.** Taken from `terra::expanse(unit = "ha", byValue = TRUE)`, the function the Area Coverage table calls, rather than measured off the exported geometry. The exported `area_ha` attribute therefore equals the reported hectares by construction. Note that `expanse` measures on the ellipsoid rather than in grid units, so a class covering thirty 100 × 100 m cells reports ≈ 30.02 ha, not exactly 30.
* **Coordinate reference system.** Shapefile and GeoPackage keep the run's projected (metric) analysis CRS, so a GIS works in the same projection the reported areas were computed in; GeoJSON and KML are reprojected to WGS84, which those formats require by specification. A GIS that re-measures the polygons planimetrically will differ from `area_ha` by the projection's own area distortion (order 0.01 % for a UTM zone), because `expanse` measures on the ellipsoid; the attribute, not the re-measurement, is the number the app reports.
* **Open ends.** The outer breaks are `-Inf` and `Inf` by construction. No GIS attribute field can hold an infinity, so the `class_min` of the first class and the `class_max` of the last are written as NULL/NA rather than as a fabricated finite limit.
* **KML attribute carriage.** The KML format itself stores only a name and a description per placemark; GDAL's `KML` driver writes nothing else, and the `LIBKML` driver that would support arbitrary fields is not present in the GDAL distributed with **sf** on Windows. The export therefore places the class label in the placemark name and serialises the full attribute record (`class`, `class_min`, `class_max`, `area_ha`, `surface`, `variable`, `method`) into the description, so nothing is silently dropped. Use GeoPackage or Shapefile when the attributes must arrive as queryable fields.
...
Predictions are collected **out-of-fold and pooled** before any metric is computed: each fold's model predicts hard classes *and* full class-probability vectors on its held-out points, and the metrics are evaluated once on the pooled set. Pooling avoids the undefined per-fold macro-metrics that arise when a spatially contiguous fold happens to contain a single class, and it lets every metric — including the probability metrics — be evaluated once over the full class set.

**Classes absent from a fold's training rows.** Spatial folds are not class-stratified (they cannot be: the blocks are defined by geometry), so a spatially clustered class can fall entirely inside one held-out block; class-stratified random folds have the same hole for a singleton class, which necessarily lands in exactly one fold. That fold's model is then fitted on data containing none of that class and can never predict it. The suite treats this as a property of the validation design rather than an error: the absent class receives probability **0** from that fold — the model's genuine posterior, since a class it never saw carries no mass — the held-out samples of that class score as misses, and the run reports which classes were affected. The consequence is that the reported producer accuracy (recall) for such a class is pessimistic by construction, and the pooled log-loss carries the corresponding penalty; overall accuracy and kappa are depressed only in proportion to the affected sample count. The remedies are the usual ones: fewer classes, quantile rather than equal-interval breaks (which distribute samples evenly and rarely strand a class), standard random k-fold when an in-domain estimate is what is wanted, or a wider spatial scope.
...
**The choice of design cannot move the reported performance metrics.** Each fold's importance shuffles run inside their **own** seeded sandbox (derived from the run seed and the fold index), so the cross-validation loop consumes exactly the same random stream whether or not importance is requested: predictions, accuracy, kappa, the confusion matrix and the per-class table are identical under both settings, and each fold's importance is independently reproducible. Without that isolation the shuffles would advance the shared stream and change the random state every later fold's model was fitted from — invisible for learners that fit deterministically (the multinomial model starts its weights at zero, Random Forest pins its engine seed, and XGBoost does not subsample at tuning depths None/Light), but real at depth **Full**, where XGBoost tunes `sample_size` and `mtry` and therefore draws random row and column subsamples. A diagnostic setting must never move a performance number, so the isolation is structural rather than dependent on which learners happen to be stochastic.
