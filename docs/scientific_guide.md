...
**Actual and Predicted surfaces.** A run that also kriges an uploaded prediction column fits that surface its own variogram by default. With **Fit Actual/Predicted Separately** unticked, the Predicted surface is kriged with the variogram of the *measured* values: the run's own Actual fit under Auto-Fit, the applied Actual model under Manual (Section 4.1). Under Auto-Fit its cross-validation refits that measured-value variogram inside each fold, from the training samples that carry a measured value, and then kriges the predictions with it.
...
<br>
This is a density-based spacing heuristic, not a guarantee of scientifically resolvable detail. Per locality the value is clamped to [1, 5000] m. The global recommendation is additionally floored at 1/300 of the larger bounding-box dimension, max(0.5 h<sub>NN</sub>, extent / 300), and clamped to [0.1, 500] m.
...
* **Wrapped (buffered):** a concave hull inflated outwards by the buffer distance (<i>D</i><sub>b</sub>) so the mapped area covers the field's outer borders.
...
Hull boundaries are not flagged. Cells near their perimeter can lose sampled points, including interior points less than half a cell diagonal from the boundary. Dynamic wrapped buffers use density-derived spacing in Auto mode, which can differ from the actual grid spacing, and are capped at 2000 m. They are not guaranteed to exceed half the actual cell diagonal.
...
**Gaussian and Matern candidates need a positive nugget in Auto-Fit.** Gaussian and the fixed Matérn 3/2 model have a quadratic leading term near the origin; the Gaussian process is especially smooth (Stein 1999). Zero-nugget models are theoretically valid, but closely spaced samples can produce poor numerical conditioning (Posa 1989; Ababou et al. 1994). Negative kriging weights can also produce overshoot without numerical singularity (Deutsch 1996). Monolith excludes zero-nugget Gau/Mat candidates throughout automated searches, including covariate and fold fits. Spherical/exponential candidates are unaffected; manual models remain as applied.
...
* **Local adaptation:** the "Optimize" button computes a separate power for every selected locality, on the rows that locality's run uses, including the **Data Subset** of a Single-split view.
* **What a stored power applies to:** every stored power carries the variable and data subset it was tuned on and is used only for that combination; a run on any other target falls back to the sidebar value. Manual per-locality powers are stored for the Actual and the Predicted surface separately, and the target switch is available in every view that computes a Predicted surface.
...
* **What a stored lambda applies to:** as with the IDW power, a stored lambda carries the variable and data subset it was tuned on, applies only to that combination, and is stored per surface; the target switch is available in every view that computes a Predicted surface.
...
The discrete error at each sampling location (<i>O<sub>i</sub> - P<sub>i</sub></i>) is drawn as coloured point markers (the Map Viewer's Point Residuals panel, exported as the Point Error Map). An IDW interpolation of coordinate-valid rows with both uploaded values present produces the Interpolated Point Errors Map in the Export Panel. Uploaded point coordinates are projected into the locality's working metric CRS before this step, so the error surface is valid for geographic uploads exactly as for projected ones.
...
* **Geometric influence:** kriging variance is generally lower near samples. The point-kriging convention can give zero variance at an exact observed location; a nugget does not automatically leave nugget variance there.
...
**Governing Factors** fits a separate regression random forest on rows complete for the target and selected predictors (minimum 10). It does not perform spatial CV or residual kriging. Permutation importance is the increase in RMSE on the DALEX evaluation sample, while the reported OOB variance explained is the forest's own OOB diagnostic. Neither establishes spatial transfer performance or causation.
...
| Fixed and Auto (Global) run grid | at most about 4 × 10⁶ bounding-box cells; Fixed absolute floor 0.1 m [2.5] | `run_regional_interpolation` |
