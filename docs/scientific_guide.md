...
- **Model CV Metrics** exports one wide row per source, matching the Model Performance card column for column, and adds two columns the card does not print: the resolved fold plan (*Standard LOOCV*, *Spatial Block CV*, or *pooled per-locality CV*) and Moran's null expectation E[I] = -1/(n - 1), which the card carries as a hover tooltip on the I cell.
