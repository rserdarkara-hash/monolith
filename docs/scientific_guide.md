...
Each locality's run grid is sized from its own boundary area by the rule in Section 2.5: a small boundary gets a fine grid and a large one a coarser grid. Sampling density is not an input to it, so two localities of equal area receive the same cell size whether one holds 30 samples or 300.

### 2.3 Auto (global)

Every locality of the run is gridded at **one cell size**: the Auto resolution of the largest boundary in the run (Section 2.5), so no locality exceeds about 100,000 cells inside its boundary and all outputs share the cell size. Before any locality is interpolated, the boundaries are built exactly as the run builds them to find that size. The grids are also snapped to the lattice of multiples of the cell size in the working CRS, so localities sharing a working CRS share one cell lattice. Localities in different UTM zones (a geographic upload spanning zones) cannot share a lattice. A Target Mapping CRS that differs from the working CRS resamples every surface onto one new lattice of the same cell size (Section 3.7).

### 2.4 Fixed (manual)

The slider value overrides the dynamic recommendation, which is useful when a specific pixel size is required for cartographic consistency or printing. Fixed grids are snapped to the same lattice of multiples of the cell size as Auto (Global), so localities that share a working CRS share their cells and the merged surface is the localities' own cells, never resampled; a merge of grids with different origins resamples every locality but the first. Only a locality whose cell size the candidate-cell guard enlarged (Section 2.5) leaves the lattice. Auto (Per Locality) sizes each locality's cells from its own area, so its grids start at their own boundary's corner and the merged surface resamples them to one cell size.
...
**<i>R</i><sub>auto</sub> and <i>R</i><sub>rec</sub> are independent quantities, and they routinely differ by an order of magnitude.** Holding the cell *count* fixed ties the cell size to the boundary rather than to the sampling: the smaller the boundary, the finer the grid, however sparse its samples. On the bundled sample data under concave-hull boundaries, Kale (79 samples, 2.71 km², <i>h</i><sub>NN</sub> = 139 m) grids at 5.20 m and Yorga (198 samples, 29.14 km², <i>h</i><sub>NN</sub> = 218 m) at 17.07 m, respectively 13 and 6.4 times finer than their <i>R</i><sub>rec</sub>, or about 1,270 and 505 cells per sample. Every engine is an exact pointwise predictor evaluated node by node, so a denser lattice introduces no bias and no spurious structure; it resolves the fitted surface more finely than the samples resolve the field, at proportionate computational cost. No mode derives the run grid from <i>R</i><sub>rec</sub>: choosing a cell size on any other ground, sample spacing included, means setting it in Fixed mode (Section 2.4).

**Cells are square and exactly the stated size.** The grid starts at the lower-left corner of the boundary's bounding box (Fixed and Auto (Global): at the lattice point below it, a multiple of the cell size) and extends by whole cells until it covers the box. Fitting a fixed number of cells into the box instead would stretch them, for example to 45.45 × 44.44 m for a requested 45 m. The Map Viewer's resolution overlay lists the cell size a run used for each locality; the run configuration records the same figures as one value, or as a range across the localities where they differ.
...
The Target Mapping CRS is the system the finished surface is written in (reprojected with `terra::project` when it differs from the working CRS), and it carries the exported rasters, the sidebar resolution suggestion (§2.1) and the ruler's projected column. The models themselves are fitted in the run's working CRS (§3.6), which coincides with the Target Mapping CRS only when the two are the same system.

**Cell size in the Target Mapping CRS.** When the Target Mapping CRS is the working CRS (the same system, however it is written), the surface is not resampled: it keeps the working grid and its values exactly and is labelled with the Target Mapping CRS definition, so an exported GeoTIFF carries that EPSG code. When it is another projected system, including the same projection on another datum, the surface is resampled (bilinear) onto square cells of the run's resolution in that system's metres, on one lattice of multiples of the cell size from its coordinate origin, so every locality lands on the same lattice and the merged surface needs no further resampling. A geographic Target Mapping CRS keeps the cell size in degrees that the reprojection chooses.

> **Recommended setting for projected data:** use the same UTM zone suited to the study area as both the Input Data CRS and the Target Mapping CRS. The working CRS is then the Target Mapping CRS, so the axis-unit rule and the scale-factor check below also apply to the grid, buffers and variogram lags, the surface is written as computed, with no final resampling, and the ruler's projected column is directly comparable with fitted ranges. A CRS is fit for the target role only where it measures correctly, and the axis-unit rule of §3.5 is necessary but not sufficient. Web Mercator's axis unit *is* the metre, and at 52° N it reports every distance 64% too long.
...
**A sill that was never observed, with a variogram still rising.** When the sill is unresolved *and* the empirical variogram is still climbing near the cutoff (the mean of the last third of the bins exceeds the mean of the third before it by more than 5% of the observed gamma span), the map carries a hedged advisory: the range and sill are weakly constrained, and this can indicate either long-range spatial dependence or non-stationarity. It is not a detection of non-stationarity, and the app does not state one. The suggested response depends on what the variogram was fitted to. On a value-scale variogram (OK; CK, which seeds its linear model of coregionalization from the direct variogram of the measured values and does not detrend), the response is to model the systematic trend with covariates (Regression Kriging) and fit the variogram to the residuals. On a residual variogram (RK, RFK) that response has already been taken, so the advisory instead reports that the fitted trend has not removed the large-scale structure and names the three causes the empirical variogram cannot separate: covariates that do not carry the large-scale variation, a trend form that cannot express the relationship, or a correlation length exceeding the locality's sampled extent. The third has no remedy within the data, because the lag cutoff is fixed at half the bounding-box diagonal; swapping covariates until the advisory clears would be fitting to noise. No engine is named in either branch, so the advice cannot be circular for whichever engine produced it. The advisory qualifies interpretation, not validity: with the range weakly identified at long lags the prediction remains the best linear unbiased one under the assumed model, and the short-lag structure that governs predictions near sample points is well constrained.
...
* **Local adaptation:** the **OPTIMIZE IDW FACTORS** button computes a separate power for every selected locality, on the rows that locality's run uses, including the **Data Subset** of a Single-split view. A locality with fewer than 5 distinct samples is not searched and nothing is stored for it: it keeps the sidebar power, and the run log names it.
...
**The RFK trend forest is seeded.** Each locality's map forest grows from the fixed seed 12345, and each cross-validation fold's forest from 12345 plus its fold number. A locality's forest, and with it its surface, kriging variance and variable importance, is therefore the same whichever other localities a run includes and in whatever order. One seed shared by forests grown on different localities' data creates no dependence between them.

**The RF Variable Importance panel** shows three measures of each locality's trend forest. The out-of-bag permutation importance appears twice: as the increase in out-of-bag MSE when a covariate is permuted, in squared target units, which orders the covariates, and divided by its standard error across trees (randomForest's %IncMSE), which grows with the tree count and is not a z-score (Strobl & Zeileis 2008). The third is IncNodePurity, the decrease in residual sum of squares from the splits on the covariate, computed on each tree's own training rows. The exported importance table has the same three columns, and Governing Factors reports the first two the same way (Section 8.8).
...
**Governing Factors** fits a separate regression random forest on rows complete for the target and selected predictors; the tab requires at least 50 such rows. It does not perform spatial CV or residual kriging.

**Importance is out-of-bag.** Each predictor's importance is Breiman's (2001) permutation importance: for every tree, the increase in the mean squared error of its out-of-bag predictions when that predictor's values are permuted, averaged over the trees (`randomForest::importance(type = 1, scale = FALSE)`, Liaw & Wiener 2002), in the target's squared units. Out-of-bag rows are the ones each tree never saw. Permuting on the rows a forest was grown on instead scores what it memorised along with what it learned: a pure-noise predictor then shows a clear importance, while out of bag it scores near zero. The unscaled value orders the factors. Beside it the table and the plot show the scaled value, the increase divided by its standard error across trees (randomForest's %IncMSE, the measure the RFK importance panel shows); it grows with the tree count, roughly with its square root, and is not a z-score (Strobl & Zeileis 2008), so scaled values compare only between runs with the same number of trees. The reported OOB variance explained is the same forest's out-of-bag diagnostic. The Governing Factors table states each row's unit and formats all metrics to four significant digits (or exact integers for whole counts). Neither statistic establishes spatial transfer performance or causation. Correlated predictors distort the ranking: the forest divides their shared signal between them, and permuting one of them alone creates combinations of values the data do not contain (Strobl et al. 2008).

The predictor with the greatest importance is selected for three profiles: partial dependence averages predictions while changing that predictor; accumulated local effects aggregate local model differences across its observed distribution; SHAP estimates feature contributions for a seeded sample of observations. The code uses DALEX's aggregated SHAP rows (`B == 0`) for the selected predictor, not the sum over individual permutations. These describe the fitted model; correlated predictors, extrapolated predictor combinations and finite Monte Carlo sampling affect their interpretation. The controls set the forest's tree count (which also sets how many trees the importance averages) and the maximum number of explained observations (Section 9.6).
...
Class limits for the agronomical styling algorithms are computed by `calc_class_breaks` (`spatial_pipeline.R`).

* **Jenks natural breaks are computed exactly, over every cell.** Natural breaks are the partition of the values into *k* classes of consecutive values that minimises the total within-class sum of squared deviations (Jenks 1967, 1977), an optimum that Fisher (1958) showed can be found exactly. `natural_breaks()` finds it by dynamic programming over all the surface's values (`Ckmeans.1d.dp`, Wang & Song 2011), deterministically and in O(n log n): about 0.7 s for a million cells. No sample is drawn, so the breaks carry no sampling error; on a synthetic million-cell surface, breaks estimated from 5,000-value samples varied by 2 to 5% of the surface SD from one draw to the next. Each break lies midway between the largest value of one class and the smallest value of the next. The partition of the data is then the optimum under the app's `[low, high)` convention: a break placed on a class's largest value would move that value into the class above, which on a variable recorded to two decimals misassigns every tied value at a class edge.
* **K-means.** `classInt`'s k-means (Hartigan & Wong 1979) draws random starts, so it runs under the app's two-sided seed sandbox (seed `12345`, caller RNG restored), making its breaks reproducible. In one dimension k-means minimises the same criterion as natural breaks; the heuristic may return a local optimum where natural breaks return the global one.
* **Each surface has its own classes.** The Actual and the Predicted surface are each classified with breaks computed from their own values, whichever map the viewer shows, so each map, legend and Area Coverage table describes its own surface and the difference between the two maps is the difference between the surfaces. **Match Scales** computes one set of breaks from both surfaces pooled and applies it to both. Supervised limits are fixed numbers and are shared by construction. The agreement table bins each measured value and its uploaded prediction with the Actual surface's classes: agreement is defined only between two columns classified the same way. Before a first run, the data column stands in for the Actual surface.
* **Areas.** The class *areas* reported in the Scientific Analysis tab are always computed by classifying the **full-resolution** raster with the breaks; each Area Coverage table states every class's range. The interactive Leaflet viewer may display a mean-aggregated preview above about 500,000 cells; this is display-only.
* **Where:** `natural_breaks()` and `calc_class_breaks` in `spatial_pipeline.R`, and `LEAFLET_DISPLAY_MAX_CELLS <- 5e5` in `server_setup.R`.
...
* **Geometry.** `terra::classify(rcl_mat, right = FALSE)`, the identical classification including the left-closed interval convention, followed by `terra::as.polygons(dissolve = TRUE)`. One feature per class present in the surface; in the Comparison view each surface is dissolved with its own classes (Section 9.4) and named in the `surface` attribute. Polygon boundaries are therefore **cell boundaries**: the zones inherit the cells of the displayed surface (the interpolation grid, resampled into the Target Mapping CRS when that is another system) and are not smoothed, contoured or generalised, because any of those would move the boundary away from where the model places it.
...
Every GeoTIFF exported by Monolith carries provenance tags, and every continuous one carries its band statistics:
* **Embedded metadata tags:** GDAL metadata tags record the run: `MONOLITH_VARIABLE` (the variable's display label), `MONOLITH_PRODUCT`, `MONOLITH_METHOD`, `MONOLITH_RUN_ID`, `MONOLITH_APP_VERSION` and `MONOLITH_CREATED`; interpolation surfaces add `MONOLITH_UNIT`, `MONOLITH_LOCALITY` and `MONOLITH_TARGET_CRS`. The classified GeoTIFF of the Classification Suite carries the same tags inside the file, beside its colour table; its category names travel in the `.tif.aux.xml` sidecar of the download.
* **Precomputed band statistics (continuous rasters):** real minimum, maximum, mean and standard deviation are written to the TIFF tags (`STATISTICS_MINIMUM`, etc.). Cell values remain unaltered at full floating-point precision (`FLT4S`), allowing external GIS software to display maps with immediate, accurate contrast stretching without recalculating statistics across the grid. The classified raster is written as 8-bit unsigned integers (`INT1U`), class codes that need no stretch.
...
| IDW power search | 0.5 to 5.0 in steps of 0.5; p = 2 when the search fails; below 5 distinct points nothing is searched or stored and the locality keeps the sidebar power [4.2] | `optimize_idw_p` (`spatial_kriging.R`), `idw_opt_item`, `OPTIMIZER_MIN_POINTS` (`spatial_pipeline.R`) |
...
| TPS lambda optimizer | below 5 distinct points nothing is searched or stored and the locality keeps the sidebar lambda; coordinates divided by the larger extent, with no per-axis rescaling (`scale.type = "unscaled"`); near-planar warning at effective df < 3.5 [4.3] | `tps_gcv_item`, `OPTIMIZER_MIN_POINTS` (`spatial_pipeline.R`), `apply_TPS` |
| RFK trend forest | `ntree = 200`; `mtry`, `nodesize` at `randomForest` regression defaults; map forest seed 12345, fold forests 12345 + fold number [7.3] | `rfk_ntree_val` (`server_execution.R`), `CV_FOLD_SEED` (`spatial_metrics.R`) |
...
| Constant | Value | Where |
|---|---|---|
| Binned styling | 5 equal-interval classes over each surface's own values (both surfaces pooled under Match Scales) [9.4] | `build_classification_params` (`server_run_config.R`) |
...
| Governing Factors | at least 50 complete rows in the tab (10 in a direct call of the function); seed 12345; trees 50 to 500 (default 100), SHAP sample 50 to 1000 (default 100) [8.8] | `compute_governing_factors` (`spatial_pipeline.R`), `gov_module.R` |

Governing Factors importance is the forest's out-of-bag permutation importance: per tree, the out-of-bag MSE after permuting a predictor minus the unpermuted out-of-bag MSE, averaged over the trees; the scaled column divides it by its standard error across trees [8.8]. Zero indicates no observed increase; a negative value means permuting the predictor reduced the out-of-bag error. The table reports OOB variance explained separately as a model-quality diagnostic.
...
Jenks binned-target breaks are the exact natural breaks over every scoped value (Section 9.4), the same computation the map classes use; quantile (type 7) and equal-interval breaks come from `classInt`. None of the three draws random numbers. The Classification Suite refuses non-metre projected Target CRSs and suitability verdicts of "block" (tested directional distortion above 1%), without an override.

The target is either an existing categorical column (soil class, land-use label) or a continuous variable discretised into ordered classes (quantile, equal-interval, or Jenks natural breaks). Intervals are left-closed `[low, high)` for consistency with the app's agronomic-class convention, with the exception of the uppermost class which is closed on both ends `[low, high]` (`include.lowest = TRUE`) so that the maximum observed value is enclosed; generated bin labels reflect this interval structure directly.
...
Each assessment point receives covariates built from that fold's training rows: numeric variables use the same kriging/IDW builder as the map, and categorical variables take the nearest training point's class. The measured covariates at held-out points are excluded. Outer-fold scoring, out-of-fold permutation importance and every tuning resample use this map input path, including the inner resamples of nested tuning. The spatial 1-NN baseline still uses only the training coordinates and labels. The map's covariate surfaces are kriged from the scoped rows that carry every covariate the model reads; they can additionally use rows with a missing target, while CV folds contain only target-labelled complete cases. CV therefore estimates the map task from the fold's available training data whether or not **Predict maps** is selected; predicting at new sites with measured covariates is a different task.
...
Strobl, C., & Zeileis, A. (2008). Danger: high power! Exploring the statistical properties of a test for random forest variable importance. In P. Brito (Ed.), *COMPSTAT 2008: Proceedings in Computational Statistics* (Vol. II, pp. 59-66). Physica-Verlag, Heidelberg.
