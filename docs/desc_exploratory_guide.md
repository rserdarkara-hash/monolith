...
A **Variable naming** switch at the top of the Analytics Engine sets every dropdown, plot axis, title and table in the suite, the Governing Factors tab included, to either the human-readable **variable labels** from your metadata mapping (default) or the raw **column names** of the uploaded file. Without a metadata mapping both options show the column names. The switch changes presentation only; calculations and fitted models use the source columns. When two columns share a label, each shows its column name in brackets.

## 1. Global Data Grouping & Discretization

The **Data Grouping & Discretization** panel at the top of the Analytics Engine defines the groups and the sub-population that the Descriptive, Correlation and PCA tabs analyse. The Descriptive and Correlation tabs follow a grouping change immediately; a PCA uses the groups active when **Run PCA** was last pressed. Governing Factors always fits its forest on every row of the dataset, whatever the grouping and the active groups.

*   **Grouping Variables (Max 5):** select categorical or numerical variables as grouping factors. Several variables combine into one group per observed combination of their levels (for example `A | Low`).
*   **Discretization:** each grouping variable gets a *Type/Binning* selector: Categorical, Numeric: Median or Numeric: Mean (two classes, at or below and above), Numeric: Tertiles (Low, Medium, High) or Numeric: Quintiles (Q1 to Q5). A numeric variable with more than 10 distinct values starts on Median; any other variable starts on Categorical. When tied values leave too few distinct tertile or quintile cut points, the variable forms a single **Low Variation** group.
*   **Select Active Groups to Compare:** once groups are defined, this dropdown isolates specific sub-populations (all groups are selected at first). A row whose grouping value is missing belongs to no group, so it is left out of the grouped analyses.
...
*   **Categorical variance:** Boxplot, Violin, Sina-style Plot. An optional **Secondary Variable (Y)** adds a second panel, so two variables can be compared across the same groups.
*   **Multivariate and spatial:** Scatterplot (with an optional **Add Trend Line**: Linear, Loess, Polynomial of degree 2 or GAM), 2D Density Heatmap, Parallel Coordinates (at least 2 variables; one line per observation), Radar Chart (at least 3 variables; one polygon per group mean), XYZ Surface (with a **Surface Fit Model**: Linear, Loess, Polynomial, GAM or Thin Plate Splines, fitted on more than 10 complete rows and drawn over the X/Y range).

A **Color Palette** selector sets the plot's colours. Plot axes and legends name the selected grouping context.

Parallel coordinates and radar normalize each variable to 0-1 using its observed values in the active groups. Observed constants map to zero; missing measurements stay missing. Unavailable variables are named in the caption. Fewer than two observed dimensions for parallel coordinates, or three for radar, produces an explanation. Radar groups with missing dimensions show their available points without a polygon.

**2.2 Significance testing (ANOVA, Kruskal-Wallis and post-hoc)**
*   On the categorical-variance plots (Boxplot, Violin, Sina), the **Group comparison test** control is a single-select button group: **None** (the default), ANOVA, Duncan's multiple range test (Duncan 1955), Tukey's HSD (Tukey 1949), and Kruskal-Wallis (Kruskal & Wallis 1952). Only one test is ever applied, and with a Secondary Variable each panel is tested on its own.
*   **Kruskal-Wallis** is the rank-based alternative when the normality indicator beside the control, or the data themselves, argue against the parametric tests; the indicator alone does not decide which test is valid. Its pairwise post-hoc comparisons behind the significance letters are Benjamini-Hochberg adjusted (Benjamini & Hochberg 1995), consistent with the FDR policy used in the correlation table.
*   **Duncan's is labelled *(liberal)* for a reason.** It controls only the comparison-wise error rate, so with *k* groups its effective family-wise error rate grows toward 1 and it separates more means than Tukey's HSD on identical data. It is provided for reproducing older agronomy literature that reports it; **Tukey's HSD is the conservative choice** for new work. See Scientific Guide Section 8.6.
*   Significance letters ('a', 'b', 'ab') are rendered directly on the plot, **Above Data** (over each group's highest value) or at the **Top of Plot**, as set by **Letter Placement**. Shared letters mean no difference was detected. ANOVA, and Tukey or Duncan with only two groups, print the *F* statistic, degrees of freedom and *p* instead of letters.

**2.3 Ghosting overlay**
*   **Enable Ghosting (Selected vs. Total)** overlays the active groups on a faded "ghosted" background representing the entire dataset. It applies to the Histogram, Density, Boxplot, Violin, Scatterplot and ECDF, and only while the active groups leave some rows out. The ghosted view draws neither significance letters nor trend lines.

**2.4 Normality testing**
*   On the Boxplot, Violin and Sina plots, a normality indicator above the test control assesses the distribution assumptions behind the parametric options, for the Primary Variable (X) in the active groups.
...
*   **Group-aware:** with more than one group, each group is tested separately and the overall test runs on the within-group residuals, so between-group differences do not register as non-normality. Otherwise it runs on the raw values. The readout states which was used.
*   **The verdict is text, not only a tooltip.** Beside the severity icon the panel prints one sentence naming the test, its statistic with that test's own symbol (*W* for Shapiro-Wilk, *D* for Lilliefors), the p-value, the significance stars, n, and the conclusion at alpha = 0.05, so it can be read without hovering and copied into a report. A non-significant result means only that the test did not reject normality. The icon's tooltip keeps the longer explanation and the per-group breakdown. See Scientific Guide Section 8.8.

**2.5 Group statistics table**

Under the plot, the **Group Statistics** table summarises the Primary Variable (X): one row per group plus a TOTAL row over every non-missing value in the active groups. A real group called TOTAL keeps its own statistics; the pooled row receives a distinct label, such as **TOTAL (pooled)**, also when copied. Beside **Count**, **Mean** and **SD** the table reports the robust counterparts, because a few outliers move the moments a long way and leave these where they are:
...
Values are computed at full precision and displayed to four significant digits, so a small-unit variable keeps its digits and a near-constant column is never shown as a constant one. On a scatterplot with a trend line the table adds, per group, the fit's **Trend_R2** (R² for Linear and Polynomial, adjusted R² for GAM) and **Trend_PVal**; a Loess fit has no R² and no F test, so it reports the **Squared Correlation (Not true R²)** between observed and fitted values instead. Groups with fewer than 5 complete pairs are left blank. Parallel Coordinates and Radar Chart have no summary table.
...
**3.4 Data table**
*   For the matrix panels, the **Correlation Matrix** table below the plot lists every pair with its exact coefficient, the raw p-value and a Benjamini-Hochberg adjusted p-value computed across all pairs shown.
*   For the Spatial Cross-Correlogram, it lists each distance bin: lag distance, pair count, cross-correlation and the standardised cross-semivariance it is derived from.
...
*   Choose at least three **Variables for PCA** under **PCA Setup**, leave **Scale & Center Data (Recommended)** ticked unless the variables share one unit and scale, and press **Run PCA**. The PCA runs on the active groups and requires at least five rows complete across the selected variables; a smaller complete population produces a persistent explanation, and rows dropped for missing values are reported in a notification. Before the PCA executes, those same complete rows are scanned, and what the scan finds is reported in up to two sections, each with its own heading and sentence (a section with nothing to report is left out):
...
*   The near-duplicate pairs are **advisory**: they stop the PCA and ask you to remove one variable of each pair from the selection, or to press **Run PCA with these variables**, because keeping both can be a legitimate choice. A constant variable is not a judgement call, so it never stops anything on its own: a variable with exactly or effectively no variance (the same rule the interpolation engines apply to covariates) is excluded from the PCA automatically, and the remaining variables keep "Scale & Center Data". A PCA with excluded variables shows a standing note above the results naming them, and every PCA figure names them in its caption.
...
*   **Biplot (2D)** colours the observations by the groups active when the PCA ran and draws the loadings as arrows, rescaled to the score range. **3D PCA Scores** shows observations only, without loading arrows, and requires at least three available components. Two-component fits remain available in the other views. Every component selection is checked against the current fit, including after rerunning with fewer variables.
*   **Loadings** are sorted by magnitude and coloured by sign. **Cumulative Variance** marks 80% with a dashed line as a reading aid, not a rule for how many components to keep.
*   The **PCA Results** table under the plot lists each component's eigenvalue, the percentage of variance it explains and the cumulative percentage.
*   **Scaling caveat (Contribution):** contribution is the share of a *component* attributable to a variable, so it is scale-sensitive by definition. The dashed line marks the average contribution, 100/*p* for *p* variables. With "Scale & Center Data" unchecked, the values are dominated by the high-variance variables and are not comparable across variables measured on different scales; the module shows an inline note under the plot controls in that case.
...
**5.1 Configuration**
*   **Target Parameter:** the numeric variable to explain.
*   **Governing Factors:** the numeric variables acting as potential influences, picked from a list with a search box and Select All / Deselect All. The target is never used as its own factor. The analysis needs at least 50 rows that carry the target and every selected factor; the message names how many rows qualify when fewer do. Rows are read as uploaded, co-located samples included (Scientific Guide Section 8.8).
...
*   **SHAP Sample Size (Max):** SHAP explanations are computationally intensive, so this caps the random subsample of explained observations (50 to 1000, default 100). Lower values run faster for quick exploration; higher values represent the dataset better.
*   Press **Run Analysis**. The results are four plots (Global Importance, SHAP Dependence, Functional Effect, and a Feature vs Target Scatterplot of the most important factor with a linear fit) above the Tabular Data Metrics table. They describe the completed run, so changing the Target Parameter afterwards does not alter them until the next run.
...
*   **ALE (Accumulated Local Effects; Apley & Zhu 2020; default):** maps the main effect of a predictor on the target by accumulating prediction differences over small intervals of the predictor's observed values. With correlated predictors it avoids partial dependence's extrapolation into combinations of values the data do not contain, and it is cheaper to compute.
*   **PDP (Partial Dependence Plot; Friedman 2001):** the marginal effect of a feature on the predicted outcome, averaged over the data.

**5.3 SHAP dependence plot**
*   Each point is the per-observation SHAP attribution (Lundberg & Lee 2017) of the most important predictor: how much that predictor shifts the model's prediction for that sample away from the dataset-mean prediction, in the target variable's own units. Across all predictors the values sum to the deviation of the sample's prediction from the mean. A loess curve traces the trend.
...
*   Interactive mode converts the `ggplot2` object into a `plotly` object, adding pan, zoom and point-specific hover readouts in the browser. Its toolbar's camera icon downloads the view as a PNG; the figure's subtitle and caption (complete-case sample, excluded variables, estimator notes) are kept under the title, so they travel with the image. A Radar Chart is redrawn as a native polar chart.
*   **3D PCA Scores** is interactive already, so its modal opens straight into the rotatable view.
