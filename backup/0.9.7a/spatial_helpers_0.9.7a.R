# spatial_helpers_0.9.7a.R - Spatial Analysis Helper Functions
...
update_progress_file <- function(l, prefix, step, total) {
  ...
  file_name <- paste0("progress_", clean_l, "_", prefix, ".txt")
  ...
  tryCatch({
    write(paste0("update_progress_file: l=", l, ", prefix=", prefix, ", pct=", pct, ", file=", file_name, ", dir=", getwd()), file = "progress_debug.log", append = TRUE)
  }, error = function(e) {
...
optimize_idw_p <- function(pts, target_var, nmax = 12) {
  ...
  rmses <- furrr::future_map_dbl(factors, function(f) {
    cv <- tryCatch({ krige.cv(form, pts, nmax = nmax, set = list(idp = f), nfold = folds, debug.level = 0) }, error = function(e) { message("CV Error for power ", f, ": ", e$message); NULL })
    if(!is.null(cv)) {
