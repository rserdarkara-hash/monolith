# Monolith: Advanced Spatial Analysis Dashboard v0.9.8
...
server <- function(input, output, session) {
  ...
  render_resid_plot <- function(cv_data_reactive, title_suffix = "") {
    renderPlot({
      ...
      if(!("residual" %in% names(cv_obj))) {
         obs_col <- grep("^var1\\.observed$|^observed$|^target\\.observed$", names(cv_obj), value = TRUE)[1]
         if (is.na(obs_col)) obs_col <- grep("\\.observed$", names(cv_obj), value = TRUE)[1]
         
         pre_col <- grep("^var1\\.pred$|^target\\.pred$|^pred$", names(cv_obj), value = TRUE)[1]
         if (is.na(pre_col)) pre_col <- grep("\\.pred$", names(cv_obj), value = TRUE)[1]
  ...
  observeEvent(rv$proceed_run, {
    ...
    promises::future_promise({
      ...
      force_globals <- list(
        ...
        apply_IDW, apply_TPS, perform_rk_cv, perform_rfk_cv, safe_run_cv, 
        optimize_idw_p, get_regional_param, clean_gstat_env, calc_area_df
      )
     ...
     if(loc == "Total (Combined)") {
       ...
       pts_sf <- if(!is.null(rv$sf)) {
         ...
         if(sf::st_is_longlat(sf_obj)) {
           c_geo <- sf::st_coordinates(sf::st_transform(sf_obj, 4326))
           utm_crs <- paste0("+proj=utm +zone=", floor((mean(c_geo[,1]) + 180) / 6) + 1, " +datum=WGS84 +units=m +no_defs", if(mean(c_geo[,2]) < 0) " +south" else "")
           sf_obj <- sf::st_transform(sf_obj, utm_crs)
         }
         sf_obj
       }
       req(pts_sf)
  ...
  build_obs_pred_plot <- function(df, title, x_lab = "Observed", y_lab = "Predicted") {
    ...
    cnames <- names(df)
    obs_col <- grep("^var1\\.observed$|^observed$|^target\\.observed$|\\.observed$", cnames, value = TRUE)[1]
    pre_col <- grep("^var1\\.pred$|^target\\.pred$|^pred$|\\.pred$", cnames, value = TRUE)[1]
