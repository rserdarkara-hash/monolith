...
# --- Improvements (v1.5a) ---
source("improvements/ui_helpers_0.8.0.R")
source("improvements/spatial_helpers_0.8.0.R")
source("improvements/theme_helpers_0.8.0.R")
...
# --- Palette Helpers ---
# Mandatory palettes for v1.2b (Simplified + Earthy colors)
...
ui <- fluidPage(
  ...
  div(class = "header-panel", style = "display: flex; justify-content: space-between; align-items: center; padding: 5px 20px;",
      h1(class = "header-title", "Monolith Spatial Analysis Dashboard (ver. 0.7.0)", style = "white-space: nowrap; margin: 0;"),
      ...
      div(class = "header-controls", style = "display: flex; align-items: center; gap: 15px; margin-left: auto;",
          ...
          div(style = "display: flex; flex-direction: column; align-items: flex-start; font-size: 0.8em; line-height: 1;",
              ...
              checkboxInput("show_borders", "Borders", TRUE),
              checkboxInput("show_scale", "Map Scale", TRUE)
          ),
      ...
      div(style="background-color: #e7f5ff; padding: 10px; border: 1px solid #a5d8ff;",
                     ...
                     conditionalPanel(condition = "input.value_type == 'resid'",
                       div(style = "background-color: #fff5f5; padding: 10px; border: 1px solid #ffc9c9; border-radius: 4px; margin-bottom: 10px;",
                         ...
                         selectInput("resid_type", "Residual Calculation:", 
                                     choices = c("Interpolated Delta (Surface Diff)" = "delta", 
                                                 "Interpolated Point Errors (Model Error)" = "point")),
                         tags$p(style="font-size: 0.8em; opacity: 0.8;", "Delta: Difference between two full surfaces. Point Error: Kriged map of local prediction errors.")
                       )
                     ),          
          ...
          conditionalPanel(condition = "input.method == 'IDW'",
              tuning_ui(
                  ...
                  extra_ui = tagList(
                      sliderInput("idw_nmax", HTML(paste0("Max Neighbors", info_tooltip("idw_nmax_info", "Limits the IDW calculation to the closest N points. This prevents distant, unrelated data from distorting local predictions. Select this BEFORE optimizing."))), min = 4, max = 50, value = 12),
                      div(style="background-color: #f8f9fa; border: 1px solid #e9ecef; border-radius: 4px; padding: 10px; color: #495057;", tableOutput("idw_metrics_table"))
                  )
              )
          ),
    ...
    mainPanel(width = 9,
      tabsetPanel(id = "main_tabs",
        tabPanel("1. Data Setup",
                ...
                tabPanel("2. Map Viewer",
                         div(style="margin-bottom:5px; display: flex; align-items: center; gap: 15px;",
                             checkboxInput("show_points_viewer", "Show Sample Points", FALSE),
                             checkboxInput("show_res_overlay", "Show Res Overlay", FALSE),
                             actionButton("show_popup_settings", "Pop-up Settings", icon = icon("cog"), class = "btn-sm"),
                             actionButton("quick_export_map", "Quick Export Map", icon = icon("camera"), class = "btn-info btn-sm", title = "Immediately send the currently viewed map to the Export Registry.")
                         ),
        ...
        tabPanel("5. Descriptive and Exploratory Suite",
                 div(style = "padding: 20px;",
                     h2("Scientific Analytics Engine"),
                     p("Explore descriptive statistics, correlation mapping, and PCA."),
                     hr(),
                     fluidRow(
                       column(12,
                         wellPanel(
                     ...
                     tabsetPanel(id = "scientific_analytics_tabs",
                                 ...
                                 tabPanel("PCA",
                                          div(style = "padding: 10px;", h4("Principal Component Analysis Placeholder"))
                                 )
                     )
                 )
        )      )
...
server <- function(input, output, session) {
  ...
  # --- Phase 2: Scientific Analytics Engine (Grouping Logic) ---
  observe({
    req(rv$user_data)
  ...
  output$desc_plot_vars_ui <- renderUI({
    ...
    tagList(
      selectInput("desc_var_x", "Primary Variable (X)", choices = valid_named),
      if (p_type %in% c("boxplot", "violin", "scatter", "density_heatmap", "xyz_surface")) {
        choices_y <- if(p_type %in% c("boxplot", "violin")) c("None" = "", valid_named) else valid_named
        sel_y <- if(p_type %in% c("boxplot", "violin")) "" else valid_cols[2]
      ...
      if (p_type %in% c("xyz_surface")) {
        selectInput("desc_var_z", "Tertiary Variable (Z)", choices = num_named, selected = num_cols[3])
      },
      if (p_type %in% c("parallel", "radar")) {
        selectInput("desc_vars_multi", "Select Variables (Min 2)", choices = num_named, multiple = TRUE, selected = head(num_cols, 3))
      },
  ...
  output$desc_main_plot <- renderPlot({
    req(rv_analytics_data(), input$desc_var_x)
    ...
    if (nrow(df_local) == 0) {
      plot(1,1,type="n", axes=F, xlab="", ylab="", main="No data selected")
      return()
    }
    ...
    if (p_type %in% core_types) {
      ...
      p <- generate_advanced_plot(df_local, vars = vars, group_col = "group_id", plot_type = p_type, xyz_fit = input$desc_xyz_fit)
    }
    ...
    if (p_type == "lagged") {
      ...
      if (p_type == "partial") {
        ...
        if(!is.null(c_vars) && length(c_vars) > 0) {
           ...
           for (v in vars_lab) {
             mod <- try(lm(as.formula(paste(v, "~", formula_rhs)), data=df_clean), silent=TRUE)
  ...
  output$var_mapping_ui <- renderUI({
    ...
    # Use existing mapping if available to populate defaults
    get_map_val <- function(target, field) {
      match <- Filter(function(x) x$actual == target, rv$mapping$vars)
      if (length(match) > 0) match[[1]][[field]] else NULL
    }

    tagList(
      lapply(seq_along(targets), function(i) {
        t <- targets[i]
        def_p_cve <- get_map_val(t, "pred")    %||% grep(paste0("^", t, "_cve$"), num_cols, ignore.case=TRUE, value=TRUE)[1]
        def_p_ss  <- get_map_val(t, "pred_ss") %||% grep(paste0("^", t, "_ss$"),  num_cols, ignore.case=TRUE, value=TRUE)[1]
        ...
        div(style="border-bottom: 1px solid #eee; padding: 10px 0; margin-bottom: 10px;",
          fluidRow(
            column(2, tags$b(t)),
            column(3, selectInput(paste0("pair_pred_cve_", i), "Best Pred (_cve)", choices = c("None", num_cols), selected = def_p_cve %||% "None")),
            column(3, selectInput(paste0("pair_pred_ss_", i),  "Split Pred (_ss)", choices = c("None", num_cols), selected = def_p_ss  %||% "None")),
  ...
  observeEvent(input$confirm_mapping, {
    ...
    for (i in seq_along(targets)) {
      ...
      cat_val <- if (is.null(raw_cat) || raw_cat == "") "Uploaded Data" else raw_cat
      
      raw_lab <- input[[paste0("pair_label_", i)]]
      lab_val <- if (is.null(raw_lab) || raw_lab == "") targets[i] else raw_lab
      
      new_vars[[length(new_vars) + 1]] <- list(
        actual = targets[i],
        pred = if (p_cve == "None") NULL else p_cve,
        pred_ss = if (p_ss == "None") NULL else p_ss,
        label = lab_val,
        category = cat_val,
  ...
  output$setup_minimap <- renderLeaflet({
    ...
    req(pts)
    leaflet(pts) %>% addProviderTiles(providers$Esri.WorldImagery) %>%
...
apply_interpolation <- function(data, target_var, method, grid_p, aux_vars, lags, method_params, l, prefix) {
  ...
  tryCatch({
    if(method == "OK") {
      ...
      for(av in aux_vars) {
        tryCatch({
          v_emp_av <- variogram(as.formula(paste(av, "~ 1")), data, width = lags$width, cutoff = lags$cutoff)
          fit_av <- robust_vgm_fit(v_emp_av, data[[av]])
          res_av <- krige(as.formula(paste(av, "~ 1")), data, grid_p, model = fit_av, debug.level = 0)
  ...
  observeEvent(input$run, {
    ...
    progressr::withProgressShiny(message = "Spatial Analysis Engine", detail = "Processing...", value = 0, {
      p <- progressr::progressor(steps = length(df_list))
...
Error in ", l, ": ", e$message)
  ...
  draw_map <- function(r_obj, lab) {
    ...
    if(is.null(r_obj) && lab != "resid_points") return(leaflet() %>% addProviderTiles(providers$Esri.WorldImagery))
