...
# --- Improvements---
source("improvements/ui_helpers_0.8.3.R")
source("improvements/spatial_helpers_0.8.3.R")
source("improvements/theme_helpers_0.8.3.R")
...
ui <- fluidPage(
  ...
  div(class = "header-panel", style = "display: flex; justify-content: space-between; align-items: center; padding: 5px 20px;",
      h1(class = "header-title", "Monolith Spatial Analysis Dashboard (ver. 0.8.3)", style = "white-space: nowrap; margin: 0;"),
...
server <- function(input, output, session) {
  ...
  output$desc_plot_vars_ui <- renderUI({
    ...
    tagList(
      ...
      if (p_type %in% c("boxplot", "violin", "scatter", "density_heatmap", "xyz_surface")) {        choices_y <- if(p_type %in% c("boxplot", "violin")) c("None" = "", valid_named) else valid_named
      ...
      if (p_type %in% c("parallel", "radar")) {
        ...
        selectInput("desc_vars_multi", label_text, choices = num_named, multiple = TRUE, selected = head(num_cols, 3))
      },
  ...
  output$desc_summary_table <- DT::renderDataTable({
    ...
    # Calculate summary statistics per group    agg_mean <- aggregate(df[[var]] ~ df$group_id, FUN=function(x) mean(x, na.rm=TRUE))
