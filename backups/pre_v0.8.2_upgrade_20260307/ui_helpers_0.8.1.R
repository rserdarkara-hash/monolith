...
generate_core_plot <- function(df, var_name, y_var = NULL, group_col = NULL, plot_type = "histogram", scatter_fit = "none") {
  library(ggplot2)
  ...
  if (plot_type %in% c("boxplot", "violin") && !is.null(y_var) && y_var != "") {
    ...
    if (plot_type == "histogram") {
      ...
      if (!is.null(scatter_fit) && scatter_fit != "none") {
         if (scatter_fit == "linear") p <- p + geom_smooth(aes(x = if(!is.null(y_var) && y_var != "") .data[[var_name]] else .data[["index_seq"]], y = if(!is.null(y_var) && y_var != "") .data[[y_var]] else .data[[var_name]], color = .data[[group_col]]), method="lm", se=FALSE)
         else if (scatter_fit == "loess") p <- p + geom_smooth(aes(x = if(!is.null(y_var) && y_var != "") .data[[var_name]] else .data[["index_seq"]], y = if(!is.null(y_var) && y_var != "") .data[[y_var]] else .data[[var_name]], color = .data[[group_col]]), method="loess", se=FALSE)
         else if (scatter_fit == "polynomial") p <- p + geom_smooth(aes(x = if(!is.null(y_var) && y_var != "") .data[[var_name]] else .data[["index_seq"]], y = if(!is.null(y_var) && y_var != "") .data[[y_var]] else .data[[var_name]], color = .data[[group_col]]), method="lm", formula = y ~ poly(x, 2), se=FALSE)
         else if (scatter_fit == "gam") p <- p + geom_smooth(aes(x = if(!is.null(y_var) && y_var != "") .data[[var_name]] else .data[["index_seq"]], y = if(!is.null(y_var) && y_var != "") .data[[y_var]] else .data[[var_name]], color = .data[[group_col]]), method="gam", formula = y ~ s(x, bs = "cs"), se=FALSE)
      }
...
generate_advanced_plot <- function(df, vars, group_col = NULL, plot_type = "qq", xyz_fit = "linear") {
  library(ggplot2)
  if (is.null(group_col) || !group_col %in% colnames(df)) {
    df$group_id <- factor("All")
    group_col <- "group_id"
  ...
  if (plot_type == "qq") {
    ...
    if (!is.null(v2) && !is.null(v3)) {
      ...
      if(nrow(df_clean) > 10) {
        ...
        mod <- NULL
        try({
          if(xyz_fit == "linear") mod <- lm(as.formula(paste(v3, "~", v1, "+", v2)), data=df_clean)
          else if(xyz_fit == "loess") mod <- loess(as.formula(paste(v3, "~", v1, "*", v2)), data=df_clean, span=0.7)
          else if(xyz_fit == "polynomial") mod <- lm(as.formula(paste(v3, "~ poly(", v1, ",2) + poly(", v2, ",2)")), data=df_clean)
          else if(xyz_fit == "gam") {
            if(requireNamespace("mgcv", quietly=TRUE)) {
              mod <- mgcv::gam(as.formula(paste(v3, "~ s(", v1, ") + s(", v2, ")")), data=df_clean)
            } else {
              mod <- lm(as.formula(paste(v3, "~", v1, "+", v2)), data=df_clean)
            }
          } else if(xyz_fit == "tps") {
            if(requireNamespace("fields", quietly=TRUE)) {
              mod <- fields::Tps(df_clean[,c(v1,v2)], df_clean[[v3]])
            }
          }
        }, silent=TRUE)
        ...
        if(!is.null(mod)) {
          if(xyz_fit == "tps") {
            grid[[v3]] <- as.vector(predict(mod, x=as.matrix(grid[,c(v1,v2)])))
          } else {
            grid[[v3]] <- as.vector(predict(mod, newdata=grid))
          }
          p <- ggplot(grid, aes(x=.data[[v1]], y=.data[[v2]], z=.data[[v3]])) +
               geom_contour_filled() + labs(title=paste("XYZ Surface (", xyz_fit, ")", sep=""))
        } else {
...
generate_correlation_heatmap <- function(df, vars, method = "pearson") {
  library(ggplot2)
...
generate_correlation_network <- function(df, vars, threshold = 0.3, method = "pearson") {
  library(ggplot2)
  if (length(vars) < 2) return(ggplot() + annotate("text", x=0, y=0, label="Need >=2 variables"))
...
generate_correlogram <- function(df, vars, method = "pearson") {
  library(ggplot2)
  if (length(vars) < 2) return(ggplot() + annotate("text", x=0, y=0, label="Need >=2 variables"))
...
check_collinearity <- function(df, vars, threshold = 0.95) {
  if (length(vars) < 2) return(list(has_collinearity = FALSE, pairs = NULL))
  
  df_clean <- na.omit(df[, vars, drop=FALSE])
  if (nrow(df_clean) < 3) return(list(has_collinearity = FALSE, pairs = NULL))
  
  # Remove near zero variance columns before cor
  vars_var <- sapply(df_clean, var)
  valid_vars <- vars[vars_var > 1e-6]
  
  if (length(valid_vars) < 2) return(list(has_collinearity = FALSE, pairs = NULL))
  
  cormat <- cor(df_clean[, valid_vars], use="pairwise.complete.obs")
  
  collinear_pairs <- data.frame(var1 = character(), var2 = character(), r = numeric())
  
  for(i in 1:(length(valid_vars)-1)) {
    for(j in (i+1):length(valid_vars)) {
      if(abs(cormat[i,j]) > threshold) {
         collinear_pairs <- rbind(collinear_pairs, data.frame(var1 = valid_vars[i], var2 = valid_vars[j], r = cormat[i,j]))
      }
    }
  }
