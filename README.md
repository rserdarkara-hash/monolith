...
# Monolith Spatial Analysis Dashboard (v1.1.3)

[![Version](https://img.shields.io/badge/version-1.1.3-6f42c1)](#)
...
* **Resolution tied to sample support:** The suggested grid spacing is half the mean nearest-neighbour distance of your samples, and the dynamic boundary buffer scales with that same spacing, so padding never claims coverage the sampling does not support. An Auto run grid is sized from each locality's mapped area (about 100,000 cells inside the boundary, clamped to 5-1000 m); **Auto (Global)** gives every locality the largest boundary's size on one shared lattice, and the size each locality was gridded at is reported after the run.
...
<details>
<summary><strong>Tested version matrix</strong>: the exact package versions Monolith 1.1.3 is developed and validated against (click to expand)</summary>
