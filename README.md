# Monolith 0.9.7c: Advanced Spatial Analysis Dashboard
...
> [!IMPORTANT]
> **Automated Package Setup in v0.9.7c:**
> You **do not need** to execute any manual `install.packages(...)` console commands.
> Sourcing the dashboard or launching `monolith_ver_0.9.7c.R` automatically triggers a smart **Auto-Installation Hook** inside `global.R`. This hook scans your environment, identifies any missing packages from the required suite, and downloads them non-interactively from the cloud CRAN repository.
> 
...
├── monolith_ver_0.9.7c.R            # Main Application Runner (Sources global.R)
├── spatial_helpers_0.9.7c.R         # Spatial math, interpolation methods & CV logic
├── ui_helpers_0.9.7c.R              # Analytics, descriptive plots & fuzzy matching helpers
├── theme_helpers_0.9.7c.R           # Theming & export configurations
├── gov_module_0.9.7c.R              # Governing Factors UI & Server Modules
├── desc_exploratory_module_0.9.7c.R # Descriptive & Exploratory Suite (Tab 5 Module)
│
...
1. Open the `monolith_ver_0.9.7c.R` file in RStudio.
...
3. Launch the Shiny app:
   ```R
   shiny::runApp("monolith_ver_0.9.7c.R")
   ```
